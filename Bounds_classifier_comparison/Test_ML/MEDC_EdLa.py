...
def Vectores_Promedio(training_data, labels):
  ...
  #Clasifico los puntos(n puntos) en una lista de k elementos(k clases)
  #El numero de clases lo obengo gracias los valores unicos
  unique_values = np.unique(labels)
  arreglo_aux = []
  for k in range(len(unique_values)): #Vector con un tamano igual al de valores unicos
    arreglo_aux.append([])

  for n in range(len(training_data_copy)):
    for k in range(len(unique_values)): #Pregunto por c/u de los valores unicos
      if labels[n] == unique_values[k]:
        arreglo_aux[k].append(training_data_copy[n])
