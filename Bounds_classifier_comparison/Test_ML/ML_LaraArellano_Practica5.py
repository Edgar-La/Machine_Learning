...
from MLPC_skl_module import run_MLPC_skl
...
datasets_names = ['Data/dataset_classifiers1.csv',		#Datasets names
					'Data/dataset_classifiers2.csv','Data/dataset_classifiers3.csv']

h = .045#.09					#how accurate will the mesh be 
k_Neighbors = 5			#Neighbors for the k-NN method
Gamma_ = 0.1			#Value for SVC
c_ = 10					#Value for SVC
folds = 10				#Folds number in cross validation
N_Epochs = 500 			#Value for perceptron and FFNN
Learning_step = .005	#Learning ratio
#--------------------------------------------------------------------------

#classifiers = ['MEDC', 'k-NN', 'SVC', 'Perceptron', 'Percep_skl', 'FFNN']						#Classifiers names
classifiers = ['MEDC', 'k-NN', 'SVC', 'Percep_skl', 'MLPC']						#Classifiers names
#classifiers = ['MEDC', 'k-NN', 'SVC', 'Percep_skl', 'FFNN', 'MLPC']
...
for n in range(len(X)):
	...
	if n==3: #Only Dataset 4 will run with 1000 epochs
		Z_FFNN = run_FFNN(X[n], y_label[n], xx[n], yy[n], Epochs=100, L_step = Learning_step)
	else:
		Z_FFNN = run_FFNN(X[n], y_label[n], xx[n], yy[n], Epochs=N_Epochs, L_step = Learning_step)
	Z.append(Z_FFNN)
	'''
	
	Z_MLPC_skl = run_MLPC_skl(X[n], y_label[n], xx[n], yy[n], Epochs=500)
	Z.append(Z_MLPC_skl)

#This method obtains the mean accuracy for every datasets and every method
#but using --CROSS VALIDATION--
get_ACC(X, y_label, classifiers, datasets_names, splits = folds, kNeighbors = k_Neighbors, Gamma = Gamma_, c = c_, Epochs=N_Epochs, L_step = Learning_step)

#This method make the plot
plotter_function(X, y_label, classifiers, datasets_names, xx, yy, Z)