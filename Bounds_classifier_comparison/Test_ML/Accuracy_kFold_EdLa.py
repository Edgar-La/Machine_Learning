...
#############################################################################
mean_ACC = []
def get_ACC(X, y_label, classifiers, datasets_names, splits = 10, kNeighbors = 5, Gamma = 0.1, c = 10, Epochs=1, L_step = .005):
	kf = KFold(n_splits = splits)

	for n in range(len(X)):
		Accuracies = []
		for w in range(len(classifiers)):
			Accuracies.append([])
		#Accuracies = [[], [], [], [], []]
		for train_index, test_index in kf.split(X[n]):
			...
			Accuracies[3].append(calculate_ACC(confusion_matrix_Percep_skl, predicted_labels_Percep_skl))
			...
			Accuracies[4].append(calculate_ACC(confusion_matrix_FFNN, predicted_labels_FFNN))
			'''
			...
			predicted_labels_MLPC = clf.predict(X_test)
			confusion_matrix_MLPC = skl.confusion_matrix(y_test, predicted_labels_MLPC)
			Accuracies[4].append(calculate_ACC(confusion_matrix_MLPC, predicted_labels_MLPC))
	...
	df.insert(0, 'datasets', ['Dataset1',		#Datasets names
					...
					'Dataset4'])
	'''
	df.insert(0, 'datasets', ['Dataset1','Dataset2','Dataset3'])					
	print(df)
